...
export default function Button({ to }) {
  ...
  // Check if the path starts with '/https', meaning it's an external link
  const isExternalLink = to.startsWith("/https");

  // Handle button click for external and internal links
  const handleClick = () => {
    if (isExternalLink) {
      // Remove the leading '/' and treat it as an external link
      const externalUrl = to.substring(1); // Remove the first character '/'
      window.open(externalUrl, "_blank"); // Open the external URL in a new tab
    } else {
      // For internal paths, navigate within the app
      navigate(to);
    }
  };

  return (
    <button onClick={handleClick} className={style.iconWrapper}>
      <svg
        width={46}
        height={42}
        viewBox="0 0 46 42"
      ...
      >
        ...
        <path
          ...
          strokeWidth="1.5"
          strokeLinecap="round"
          className={style.iconpathStroke}
        />
      </svg>
    </button>
  );
