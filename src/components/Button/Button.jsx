...
export default function Button({ to }) {
  ...
  const handleClick = () => {
    if (to.startsWith("/http") || to.startsWith("/https")) {
      ...
      console.error("Invalid or undefined 'to' prop in Button component");
    }
  };
  return (
    <button onClick={handleClick} className="inline-flex cursor-pointer  group">
      <svg
        width={46}
        height={42}
        viewBox="0 0 46 42"
      ...
      >
        <path
          ...
          className="fill-gray-500 stroke-gray-500 group-hover:fill-white group-hover:stroke-white transition-all duration-300 "
        />
        <rect
          y="21.6981"
          width={1}
          ...
          className="fill-gray-500 group-hover:fill-white transition-all duration-300 "
        />
        <path
          ...
          strokeWidth="1.5"
          strokeLinecap="round"
          className="stroke-gray-500 group-hover:stroke-white transition-all duration-300 group-hover:scale-105"
        />
      </svg>
    </button>
  );
